...
public class backup_search {    
    int diff=0;
    int total_journey_time;
    public ArrayList<ComboFlight> search(String origin_city,String journey_date,int no_of_tickets){     
     ...
     String flight2_day;
        ...
        ArrayList a1 = dm.readSpiceJet();
        ArrayList a2 = dm.readSilkAir();
                    ...
                    if(flight1_date >= Integer.parseInt(f1.getEffectiveFrom()) && flight1_date <= Integer.parseInt(f1.getEffectiveTill())) 
                    {
                        ...
                        for(int j = 0; j < a2.size(); j++) 
                        {
                            ...
                            if(f2.getdepartureCity().toLowerCase().contains(flight1_arrival_city.toLowerCase())) 
                            {
                                ...
                                int flight2_arrival_time = Integer.parseInt(f2.getarrivalTime());
                                ...
                                if(flight2_date >= Integer.parseInt(f2.getEffectiveFrom()) && flight2_date <= Integer.parseInt(f2.getEffectiveTill()))
                                {
                                   try{
                                    ...
                                    if(ans.equals("sameday"))
                                    {
                                        ...
                                        {
                                          total_journey_time=get_journey_time(flight1_departure_time,flight1_arrival_time)+diff+get_journey_time(flight2_departure_time,flight2_arrival_time); 
                                            
                                          dm.read_spiceJetArray();
                                          dm.read_silkAirArray();
                                          if(dm.a[i]>=no_of_tickets && dm.b[i]>=no_of_tickets)
